...
// Legacy compatibility API for old components (ShoppingListMasterView)
export type ShopItem = any;

export const readList = () => {
  return { items: useShoppingListStore.getState().items };
};

export const setItems = (items: any[]) => {
  useShoppingListStore.getState().replaceItems(items);
  window.dispatchEvent(new CustomEvent('shopping:list:updated'));
};

export const toggleChecked = (id: string) => {
...
export const updateItem = (id: string, updates: any) => {
  useShoppingListStore.getState().updateItem(id, updates);
  window.dispatchEvent(new CustomEvent('shopping:list:updated'));
};
...
export const readOptions = () => {
  return { scopeByWeek: false };
};

export const writeOptions = (opts: any) => {
  window.dispatchEvent(new CustomEvent('shopping:options:updated'));
};

export const setWeekScope = (enabled: boolean) => {
  // No-op for now
};