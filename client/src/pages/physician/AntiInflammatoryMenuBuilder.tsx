...
export default function AntiInflammatoryMenuBuilder() {
  ...
  const { board: hookBoard, loading: hookLoading, error, save: saveToHook, source } = useWeeklyBoard("2", weekStartISO);
  ...
  useEffect(() => {
    const infoSeen = localStorage.getItem("anti-inflammatory-menu-builder-info-seen");
    if (infoSeen === "true") {
      setHasSeenInfo(true);
    }
    
    const dailyTotalsInfoSeen = localStorage.getItem("anti-inflammatory-menu-builder-daily-totals-info-seen");
    if (dailyTotalsInfoSeen === "true") {
      setHasSeenDailyTotalsInfo(true);
    }

    const savedStep = localStorage.getItem("anti-inflammatory-menu-builder-tour-step");
  ...
  useEffect(() => {
    ...
    if (tourStep === "breakfast" && lists.breakfast.length > 0) {
      ...
      setTourStep("dinner");
      localStorage.setItem("anti-inflammatory-menu-builder-tour-step", "dinner");
    } else if (tourStep === "dinner" && lists.dinner.length > 0) {
      setTourStep("snacks");
      localStorage.setItem("anti-inflammatory-menu-builder-tour-step", "snacks");
    } else if (tourStep === "snacks" && lists.snacks.length > 0) {
      setTourStep("complete");
      localStorage.setItem("anti-inflammatory-menu-builder-tour-step", "complete");
    }
    ...
    >
      ...
      <Dialog open={showDailyTotalsInfo} onOpenChange={(open) => {
        if (!open) {
          ...
          setHasSeenDailyTotalsInfo(true);
          localStorage.setItem("anti-inflammatory-menu-builder-daily-totals-info-seen", "true");
        }
      }}>
