...
import { lifestyleDietPickerConfig } from "@/data/lifestyleDietPickerConfig";
...
export default function LifestyleDietMenuBuilder() {
  ...
  const AI_MEALS_CACHE_KEY = "lifestyle-ai-meal-creator-cached-meals";
  ...
  useEffect(() => {
    const infoSeen = localStorage.getItem("lifestyle-menu-builder-info-seen");
    if (infoSeen === "true") {
      setHasSeenInfo(true);
    }
    
    const dailyTotalsInfoSeen = localStorage.getItem("lifestyle-menu-builder-daily-totals-info-seen");
    if (dailyTotalsInfoSeen === "true") {
      setHasSeenDailyTotalsInfo(true);
    }

    const savedStep = localStorage.getItem("lifestyle-menu-builder-tour-step");
  ...
  const handleInfoModalClose = () => {
    ...
    setHasSeenInfo(true);
    localStorage.setItem("lifestyle-menu-builder-info-seen", "true");
  };
  ...
  useEffect(() => {
    ...
    if (tourStep === "breakfast" && lists.breakfast.length > 0) {
      setTourStep("lunch");
      localStorage.setItem("lifestyle-menu-builder-tour-step", "lunch");
      ...
      setTourStep("dinner");
      localStorage.setItem("lifestyle-menu-builder-tour-step", "dinner");
    } else if (tourStep === "dinner" && lists.dinner.length > 0) {
      setTourStep("snacks");
      localStorage.setItem("lifestyle-menu-builder-tour-step", "snacks");
    } else if (tourStep === "snacks" && lists.snacks.length > 0) {
      setTourStep("complete");
      localStorage.setItem("lifestyle-menu-builder-tour-step", "complete");
    }
  ...
  // Empty state when no diet selected
  if (!selectedDiet) {
    return (
      <motion.div 
      ...
      >
        ...
        <div className="max-w-2xl mx-auto p-4 md:p-8">
          <div className="bg-black/30 backdrop-blur-lg border border-white/20 rounded-2xl p-8 text-center shadow-2xl mb-8">
          ...
          <div className="bg-black/30 backdrop-blur-lg border border-white/20 rounded-2xl p-6 mb-8">
          ...
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-8 text-center">
            <p className="text-xl font-semibold text-white mb-2">⚠️ Choose a diet to continue</p>
            <p className="text-sm text-white/70">
              Select a lifestyle diet from the dropdown above to start building your meal plan
            </p>
          </div>
        </div>
      </motion.div>
    ...
    >
          ...
          <div>
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <h1 className="text-white/95 text-lg sm:text-xl font-semibold">Lifestyle Diet Meal Board</h1>
                <button
              ...
              <div className="flex items-center gap-2">
                <Select value={selectedDiet} onValueChange={setSelectedDiet}>
                  <SelectTrigger className="w-[160px] bg-white/20 border-white/40 text-white text-xs [&>span]:text-white">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="mediterranean">Mediterranean</SelectItem>
      ...
      {board && (() => {
        ...
        return (
          <ShoppingAggregateBar
            ...
            source={`Lifestyle Meal Plan (${formatWeekLabel(weekStartISO)})`}
            sourceSlug="lifestyle-meal-board"
          />
        );
      })()}
      ...
      {showInfoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-black/30 backdrop-blur-lg border border-white/20 rounded-2xl p-6 max-w-md w-full shadow-xl">
            <h3 className="text-xl font-bold text-white mb-4">How to Use Lifestyle Diet Menu Builder</h3>
      ...
      <Dialog open={showDailyTotalsInfo} onOpenChange={(open) => {
        if (!open) {
          ...
          setHasSeenDailyTotalsInfo(true);
          localStorage.setItem("lifestyle-menu-builder-daily-totals-info-seen", "true");
        }
      }}>
