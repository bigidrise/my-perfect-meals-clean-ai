  ...
  try {
    ...
    for (let index = 0; index < meals.length; index++) {
      ...
      const processedMeal: FridgeRescueMeal = {
        ...
        ingredients: meal.ingredients || fridgeItems.map(item => ({ name: item, quantity: "as needed", unit: "" })),
    ...
    const fallbackMeals: FridgeRescueMeal[] = [
      {
        ...
        ingredients: fridgeItems.map(item => ({ name: item, quantity: "as needed", unit: "" })),
      ...
      },
      {
        ...
        ingredients: fridgeItems.map(item => ({ name: item, quantity: "1 portion", unit: "" })),
      ...
      },
      {
        ...
        ingredients: fridgeItems.map(item => ({ name: item, quantity: "to taste", unit: "" })),
